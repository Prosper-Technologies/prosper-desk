...
async function testEndpoints() {
  ...
  try {
    ...
    if (listResponse.ok) {
      ...
      console.log(
        `   Pagination: Page ${data.pagination?.page || 1} of ${data.pagination?.total_pages || 1}`
      )
    } else {
    ...
    if (ticketId) {
      ...
      if (commentResponse.ok) {
        ...
        console.log(
          `   Added comment: ${data.data.content.substring(0, 50)}...`
        )
      } else {
        console.log(
          `❌ POST /tickets/:id/comments - Failed (${commentResponse.status})`
        )
      ...
      if (commentsResponse.ok) {
        ...
        console.log(
          `❌ GET /tickets/:id/comments - Failed (${commentsResponse.status})`
        )
    ...
    console.log(
      "- Ensure the development server is running on http://localhost:4000"
    )
